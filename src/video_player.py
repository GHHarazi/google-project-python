...
class VideoPlayer:
    ...
    def show_all_videos(self):
        """Returns all videos."""
        vids_list = self._video_library.get_all_videos()
        sorted_list = sorted(vids_list, key=lambda i: i.title)
        print("""Here is a list of all available videos:""")
        for vid in sorted_list:
            tag_list = list(vid.tags)
            tag_list = '[%s]' % ' '.join(map(str, tag_list))
            print(vid.title, "(" + vid.video_id + ")", tag_list)
